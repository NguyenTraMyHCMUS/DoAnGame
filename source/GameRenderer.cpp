...
#include <string>
// Include đầy đủ các file liên quan đến hiệu ứng
#include "Effects/EffectManager.h"  // Đảm bảo include này trước
#include "Effects/IEffectManager.h"
#include "Effects/LineClearEffect.h"

GameRenderer::GameRenderer(sf::RenderWindow& window, ResourceManager& resources)
    : window(window), resources(resources), _timer(0.0f) {
    // Khởi tạo EffectManager thông qua interface
    _effectManager = std::unique_ptr<IEffectManager>(new EffectManager());
}
...
void GameRenderer::display() {
        // Vẽ hiệu ứng trước khi hiển thị
    if (_effectManager) {
...
void GameRenderer::initLineClearEffect(const std::vector<int>& lines) {
    if (_effectManager) {
        _effectManager->initLineClearEffect(lines);
    }
}

bool GameRenderer::hasActiveLineClearEffect() const {
    if (_effectManager) {
        return _effectManager->hasActiveLineClearEffect();
    }
    return false;
}

std::vector<int> GameRenderer::getActiveLinesToClear() const {
    if (_effectManager) {
        return _effectManager->getActiveLinesToClear();
    }
    return {};
}